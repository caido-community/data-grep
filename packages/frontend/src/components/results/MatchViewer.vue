...
const responseRaw = ref<string | undefined>(undefined);
...
const highlightedRequest = computed(() => {
  ...
  if (match.source !== "request")
    return DOMPurify.sanitize(escapeHtml(requestRaw.value));
...
const highlightedResponse = computed(() => {
  if (responseRaw.value === undefined || responseRaw.value === "") return "";
  if (match.source !== "response")
    return DOMPurify.sanitize(escapeHtml(responseRaw.value));
...
const loadData = async () => {
  ...
  try {
    const result = await sdk.backend.getRequestData(match.requestId);
...
<template>
  ...
  >
    <div class="flex flex-col gap-3">
      <div
        class="px-3 py-2 bg-zinc-800 rounded-md text-[13px] flex items-center justify-between"
      >
        <div class="text-gray-400">
          Match found in
          <span class="text-red-500 font-semibold mx-1">{{
            match.source
          }}</span>
      ...
      <table v-else class="w-full border-collapse table-fixed">
        <thead>
          <tr>
            <th
              class="text-left py-1.5 text-[11px] font-semibold text-gray-400 w-1/2"
            >
              <i class="fas fa-arrow-up mr-1"></i>Request
              <span v-if="match.source === 'request'" class="text-red-500 ml-1"
                >(match)</span
              >
            </th>
            <th
            ...
            >
              ...
              <span v-if="match.source === 'response'" class="text-red-500 ml-1"
                >(match)</span
              >
            </th>
          </tr>

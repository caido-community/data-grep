  ...
  >
    <div class="flex flex-col gap-3">
      <p class="text-xs text-gray-400">
        Write JavaScript to transform each match. Receives
        <code class="text-blue-400">match</code> (string), return transformed
        value or <code class="text-blue-400">null</code> to skip.
      </p>
      ...
      <div class="text-xs text-gray-500 bg-zinc-900 p-2 rounded font-mono">
        ...
        <code class="text-blue-400">return match.toUpperCase();</code> —
        Uppercase<br />
        <code class="text-blue-400">return match.split('@')[1];</code> — Extract
        domain<br />
        <code class="text-blue-400"
          >return match.endsWith('.com') ? match : null;</code
        >
        — Filter
      </div>
    </div>
